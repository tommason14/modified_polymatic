...
sub readArgs
{
    ...
    while (scalar(@args) > 0)
    {
        ...
        # Molecules numbered as in barriers.in
        elsif ($flag eq "-n"){
            $fileMols = shift(@args);
        }

        # File of reaction barriers
        elsif ($flag eq "-b"){
            $fileBarriers = shift(@args);
        }

        # File of molecules included
        elsif ($flag eq "-m"){
            $molsIncluded = shift(@args);
        }
...
sub findPair
{
    ...
    if ($closest == 0) {
        ...
        # molecule number from barriers.in (1 or 2)
        my $mol1 = $numberedMols{$sys{'atoms'}{'mol'}[$pair[0]]};
        my $mol2 = $numberedMols{$sys{'atoms'}{'mol'}[$pair[1]]};
        # names
        my $molecule1 = `grep mol$mol1 $fileBarriers | awk '{print toupper(\$2)}'`;
        my $molecule2 = `grep mol$mol2 $fileBarriers | awk '{print toupper(\$2)}'`;
        ...
        # increment numbers in input file
        my @molnums = ($mol1, $mol2);
        if ( grep(/^1$/,@molnums)) {
          Polymatic::incrementMolIncluded(1, $molsIncluded);
        }
        if ( grep(/^2$/,@molnums)) {
          Polymatic::incrementMolIncluded(2, $molsIncluded);
        }

        printf "  Pair: %.2f A (%d,%d) (%s - %s)\n", $closest, $pair[0], $pair[1], $molecule1, $molecule2;
        return @pair;
    }
}
